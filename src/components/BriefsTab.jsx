...
// brief: { id, title, details?, status:'pending'|'submitted', preparedDate, submittedDate }
export default function BriefsTab({ c, onUpdate }) {
  ...
  const add = () => {
    ...
    onUpdate({
      ...
      briefs: [...briefs, {
        ...
        details: details.trim(), preparedDate: todayStr, submittedDate: "",
      }],
    });
    setTitle("");
    setDetails("");
  ...
  const updateBrief = (id, patch) => onUpdate({
    ...c,
    briefs: briefs.map((b) => (b.id === id ? { ...b, ...patch } : b)),
  });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 space-y-2">
        ...
        <textarea className="input text-sm w-full min-h-[60px]" placeholder="상세 내용 메모 (쟁점, 보완할 자료, 제출 전 확인사항 등)"
      ...
      <div>
        ...
        {pending.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {pending.map((b) => (
              <div key={b.id} className="rounded-lg px-3 py-2.5 border border-amber-200 bg-amber-50/40">
        ...
        {submitted.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {[...submitted].sort((a, b) => (b.submittedDate || "").localeCompare(a.submittedDate || "")).map((b) => (
              <div key={b.id} className="rounded-lg px-3 py-2.5 border border-slate-100 bg-slate-50">
                <div className="flex items-center gap-3">
